...
setTimeout(() => {
    const invalidCredentialsMessageContainer = $$(
        '#invalidCredentialsMessageContainer'
    );
    const redirectedMessageContainer = $$('#redirectedMessageContainer');
    if (invalidCredentialsMessageContainer)
        invalidCredentialsMessageContainer.classList.add('visible');
    if (redirectedMessageContainer)
        redirectedMessageContainer.classList.add('visible');
}, 250);
...
setTimeout(() => {
    ...
    const redirectedMessageContainer = $$('#redirectedMessageContainer');
    if (invalidCredentialsMessageContainer)
        invalidCredentialsMessageContainer.classList.remove('visible');
    if (redirectedMessageContainer)
        redirectedMessageContainer.classList.remove('visible');
}, 5000);
